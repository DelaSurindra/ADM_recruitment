        ...
        if (count($collection) == 1) {
            ...
            try {
                Validator::make($rows, [
                    ...
                    '*.2' => ["required", "regex:/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/"], // email
                    '*.3' => ["required",  "regex:/^[0-9]*$/"], // birth date
                    '*.4' => ["required", "regex:/^[a-zA-Z]*$/"], // gender
                    '*.5' => ["required", "regex:/^\d{11,13}$/"], // phone
                    '*.6' => ["required"], // location
                    '*.7' => ["required", "regex:/^[a-zA-Z0-9 ]*$/"], // univ
                    '*.8' => ["required", "regex:/^[a-zA-Z0-9]*$/"], // degree
                    '*.9' => ["required", "regex:/^[a-zA-Z0-9 ]*$/"], // faculty
                    '*.10' => ["required"], // major
                    '*.11' => ["required", "regex:/^\d{4}$/"], // start year
                    '*.12' => ["required", "regex:/^\d{4}$/"], // end year
                    '*.13' => ["required", "regex:/^[0-9.]*$/"], // gpa
                ], $this->messages)->validate();
                
                for ($i=0; $i < count($rows); $i++) {
                    if (\strtolower($rows[$i][4]) == "male" || \strtolower($rows[$i][4]) == "female") {
                        if (\strtolower($rows[$i][8]) == "d3" || \strtolower($rows[$i][8]) == "s1" || \strtolower($rows[$i][8]) == "s2") {
                            if ($rows[$i][13] > 4) {
                                $dataRes = [
                                    'email' => $rows[$i][2],
                                    'status' => 'Failed',
                            ...
                            $dataRes = [
                                'email' => $rows[$i][2],
                                'status' => 'Failed',
                                'note' => 'Degree must d3 or s1 or s2'
                            ];
                            array_push($this->data, $dataRes);
                        }
                    }else{
                        $dataRes = [
                            'email' => $rows[$i][2],
                            'status' => 'Failed',
                            'note' => 'Gender must male or female'
                        ];
                        array_push($this->data, $dataRes);
                    }
                }
